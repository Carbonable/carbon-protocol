...
mod CarbonableMinter {
    ...
    struct Storage {
        _project: felt,
        _payment: felt,
        _public_sale_open: felt,
        _max_buy: felt,
        _unit_price: u256,
        _max_supply: u256,
        _reserved_supply: u256,
        _merkle_root: felt,
        _claimed_slots: LegacyMap::<felt, felt>,
    }

    // Events
    #[event]
    fn PreSaleOpen(time: felt) {}

    #[event]
    fn PreSaleClose(time: felt) {}

    #[event]
    fn PublicSaleOpen(time: felt) {}

    #[event]
    fn PublicSaleClose(time: felt) {}

    #[event]
    fn SoldOut(time: felt) {}

    #[event]
    fn Airdrop(address: felt, quantity: felt, time: felt) {}

    #[event]
    fn Buy(address: felt, amount: u256, quantity: felt, time: felt) {}
    ...
    fn initializer(
        ...
        unit_price: u256,
        max_supply: u256,
        reserved_supply: u256,
    ) {
        // [Effect] Set storage variables
        _project::write(project);
        _payment::write(payment);
        _max_buy::write(max_buy);
        _unit_price::write(unit_price);
        _max_supply::write(max_supply);
        _reserved_supply::write(reserved_supply);
    ...
    fn pre_sale_open() -> felt {
        starknet::is_not_zero(_merkle_root::read())
    }

    fn public_sale_open() -> felt {
        _public_sale_open::read()
    }
    ...
    fn reserved_supply() -> u256 {
        _reserved_supply::read()
    }
    ...
    fn claimed_slots() -> felt {
        _claimed_slots::read()
    }
}
//     // Computes
//     fn whitelisted_slots(
