...
TIMES="2 1711965600 2469348000"
ABSORPTIONS="2 0 1187142000000"
...
deploy() {
    ...
    if [[ $debug == "true" ]]; then
        printf "deploy %s %s %s %s %s \n" "$class_hash" "$NAME" str:"$SYMBOL" "$DECIMALS" "$OWNER" >> debug_project.log
    fi
...
setup() {
    #contract=$(deploy)
    #sleep 5
    contract="0x00130b5a3035eef0470cff2f9a450a7a6856a3c5a4ea3f5b7886c2d03a50d2bf"

    if [[ $debug == "true" ]]; then
        printf "invoke %s set_project_value u256:%s %s \n" "$contract" "$SLOT" "$PROJECT_VALUE" >> debug_project.log
    fi
    ...
    if [[ $debug == "true" ]]; then
        printf "invoke %s set_certifier u256:%s %s \n" "$contract" "$SLOT" "$OWNER" >> debug_project.log
    fi
