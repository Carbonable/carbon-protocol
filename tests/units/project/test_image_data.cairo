...
func test_image_data{
    ...
    let ss = 'base64,PCFET0...C9odG1sPg==';
    let (str) = StringCodec.ss_to_string(ss);

    CarbonableProject.set_image_data(image_data_len=str.len, image_data=str.data);

    let (len, array) = CarbonableProject.image_data();
...
@external
func test_image_data_revert_not_owner{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr
}() {
    alloc_locals;
