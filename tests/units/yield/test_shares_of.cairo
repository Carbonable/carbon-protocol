...
func test_shares_of{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() {
    ...
    let (shares) = CarbonableYielder.shares_of(address=context.signers.anyone, precision=10);
    assert shares = ten;  // 10 / 10 = 1 = 100%

    let (shares) = CarbonableYielder.shares_of(address=context.signers.anyone, precision=100);
    assert shares = hundred;  // 100 / 100 = 1 = 100%

    let (shares) = CarbonableYielder.shares_of(address=context.signers.admin, precision=100);
